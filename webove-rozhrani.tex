...
z~nich bylo, že původní aplikace mohla kvůli své designové nevytříbenosti
jen těžko sloužit pro širokou veřejnost.
...
úrazu je zde však podpora, která se momentálně omezuje pouze na prohlížeče na
bázi Chromia.
...
nahrávek na segmenty (sekce \ref{sec:segmenty}) přestala být potřeba ukládat nahrávky aktuální.
...
Až do května 2021 jsem používal pro nucené zarovnání nástroj \texttt{HVite} ze
...
\item(Délka:
    časová pozice od začátku slova, kde končí poslední vlastní hláska
    (tedy kde začíná ticho nebo následující slovo).
\item{Manuálně přepsané:
    pravdivostní hodnota odlišující manuálně přepsaná slova od automaticky
    přepsaných.
}
\item{Confidence measure\footnote{Od přechodu na zarovnávání založené na
    Montreal Forced Aligner se již nepoužívá.}:
    míra jistoty, se kterou bylo slovo predikováno (týká se pouze automaticky
    přepsaných slov).
}
\item{Ticho\footnote{Zastaralý údaj nahrazený délkou slova.}:
...
explicitně háčkem a např. sekvence {\em n i} se vždy vyjádří jako {\em ny}.
...
\item{
    ...
    \item{
        ...
        \begin{itemize}
            ...
            \item{\texttt{subs}: seznam zarovnaných slov,}
        \end{itemize}
    }
    \item{
...
\}\end{alltt}
    ...
    Seznam zarovnaných slov je v~tomto formátu:
    \begin{alltt}[ 
  \{
    fonet:{\em řetězec},
    wordform:{\em řetězec},
...
s~odpovídajícím úsekem audia. Aparát pro nucené zarovnání zahrnuje spustitelné
binární soubory z~balíčku HTK, akustický model a uložené nahrávky
v~parametrizovaném formátu MFCC. Na jaře 2021 jsem implementoval zarovnávání
modernějším nástrojem Montreal Forced Aligner a nasadil jako separátní webovou
službu. Ta běží v~rámci Lindatu a přijímá requesty metodou POST se dvěma
parametry: audiem v~libovolném formátu, který sox dokáže dekódovat, a přepisem
ve formě prostého textu. Odpovědí je zarovnání ve formátu
TextGrid\footnote{\url{https://www.fon.hum.uva.nl/praat/manual/TextGrid\_file\_formats.html}}.
Od té doby \texttt{subsubmit} přijímá zarovnaný text jako parametr, zatímco
dříve bylo zarovnání jeho hlavní úlohou.
O převod z~formátu TextGrid do seznamu zarovnaných slov se stará front-end.
